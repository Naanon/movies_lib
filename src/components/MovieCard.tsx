  ...
  return (
    <div className={`${rest.posterWidth} text-white text-center mb-10 flex flex-col justify-between bg-black-400 p-4 rounded-xl`}>
      ...
      <p className="mb-4 flex items-center justify-center gap-1.6">
        <FaStar className="text-2xl text-blue-250" /> {rest.vote_average}
      </p>
      {rest.showLink && (
        <Link
          to={`/movie/${rest.id}`}
          className="bg-blue-250 border-2 border-blue-250 rounded text-black py-4 items-center duration-400 font-semibold hover:bg-transparent hover:text-blue-250"
        >
          Details
        </Link>
      )}
